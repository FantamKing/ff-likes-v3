...
def home():
    return jsonify({
        "message": "Free Fire Likes API - DEBUG MODE",
        "status": "active",
        "credits": {
            "Developer": "God",
...
def handle_requests():
    try:
        ...
        # Send likes
        results = asyncio.run(send_multiple_requests(uid, server_name, int(like_count)))
        
        # Get after likes
        after = make_request(encrypted_uid, server_name, token)
        if after is None:
            return jsonify({"error": "Failed to get updated profile"}), 500
